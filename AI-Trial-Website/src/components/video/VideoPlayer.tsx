import { IconDownload, IconExternal, IconVideo } from "../ui/Icons";
...
export function VideoPlayer({ src, downloadName, label, className = "" }: VideoPlayerProps) {
  return (
    <div className={`flex flex-col overflow-hidden border border-line bg-base ${className}`}>
      <div className="flex items-center gap-2 border-b border-line bg-raised px-3 py-2">
        <IconVideo size={16} className="text-mint" />
      ...
      <video
        ...
        playsInline
        preload="metadata"
        className="max-h-[60vh] w-full bg-night"
      />
    </div>
  );
}
