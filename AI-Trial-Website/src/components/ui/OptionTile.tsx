...
export function OptionTile({
  ...
  return (
    <button
      ...
      className={`group relative flex w-full items-start gap-3 border p-4 text-left transition-colors duration-150 ${
        selected
          ? "border-mint bg-mint/10 shadow-[0_0_0_1px_var(--color-mint)_inset]"
      ...
      {icon && (
        <span
          className={`inline-flex h-10 w-10 shrink-0 items-center justify-center border transition-colors ${
            selected ? "border-mint/50 bg-mint/15 text-mint" : "border-line bg-base text-ink-dim group-hover:text-ink"
          }`}
        >
          {icon}
        </span>
